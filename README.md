...
- エクスプレッションペダル処理:
  - デッドゾーン適用 → 適応型EMAフィルタ → スケーリング
  - 変化閾値: 設定範囲の1%以上で送信（最小1）
  - 送信レート制限: 最大100メッセージ/秒/ペダル
